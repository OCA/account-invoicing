...
class TestStockReturnPicking(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.env.user.company_id.customer_return_fees_product_id = cls.env.ref(
            "sale_stock_picking_return_fees_invoicing."
            "product_customer_return_fees")

        cls.partner = cls.env["res.partner"].create(
            {"name": "Partner", "charge_customer_stock_return": False}
        )
    ...
    def test_00(self):
        """
        Data:
            A customer not charged with return fees
        ...
        Expected result:
             ...
             * contains 2 lines
             * be configured to not charge the customer with return fees
        """
        self.assertFalse(self.partner.charge_customer_stock_return)
        wizard = self._create_return_wizard()
        self.assertTrue(wizard.is_customer_return)
        self.assertFalse(wizard.charge_customer_stock_return)
        for line in wizard.product_return_moves:
            self.assertFalse(line.charge_customer_stock_return)

    def test_01(self):
        """
        Data:
            A customer charged with return fees
            A delivered SO with 2 lines
        Test case:
            Create a stock return wizard
        Expected result:
             ...
             * marked as customer return
             * contains 2 lines
             * be configured to charge the customer with return fees
        """
        self.partner.charge_customer_stock_return = True
        wizard = self._create_return_wizard()
        self.assertTrue(wizard.is_customer_return)
        self.assertTrue(wizard.charge_customer_stock_return)
        for line in wizard.product_return_moves:
            self.assertTrue(line.charge_customer_stock_return)
    ...
    def test_02(self):
        ...
        fees_line = self.so.order_line.filtered("is_customer_return_fees")
    ...
    def test_03(self):
        ...
        Expected result:
            SO has no new lines
        """
        self.partner.charge_customer_stock_return = False
        self.assertEqual(2, len(self.so.order_line))
        picking = self._create_return_picking()
        self.assertEqual(2, len(self.so.order_line))
        self._process_picking(picking)
    ...
    def test_04(self):
        ...
        self.assertEqual(2, len(self.so.order_line))
        wizard = self._create_return_wizard()
        wizard.product_return_moves[0].charge_customer_stock_return = False
    ...
    def test_05(self):
        ...
        self.assertEqual(2, len(self.so.order_line))
        wizard = self._create_return_wizard()
        wizard.product_return_moves[0].charge_customer_stock_return = True
        res = wizard.create_returns()
        picking = self.env["stock.picking"].browse(res["res_id"])
        self.assertEqual(2, len(self.so.order_line))
        self._process_picking(picking)
