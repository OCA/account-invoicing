...
class TestAccountMoveSalePartner(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.env.company.invoice_group_by_sale_partner = True
    ...
    def test_single_sale_order_invoice(self):
        ...
        self.assertEqual(so.invoice_ids.partner_sale_id, self.partner1)
    ...
    def test_two_sales_different_customer_same_invoice_partner(self):
        ...
        self.assertEqual(so1.invoice_ids.partner_sale_id, self.partner1)
        self.assertEqual(so2.invoice_ids.partner_sale_id, self.partner2)
    ...
    def test_two_sales_same_customer_and_invoice_partner(self):
        ...
        (so1 + so2)._create_invoices()
        self.assertEqual(so1.invoice_ids.partner_sale_id, self.partner1)
        self.assertEqual(so2.invoice_ids.partner_sale_id, self.partner1)
        self.assertEqual(so1.invoice_ids, so2.invoice_ids)

    def test_invoice_group_by_sale_partner(self):
        self.env.company.invoice_group_by_sale_partner = False
        so1 = self._create_sale_order(self.partner1, self.invoice_partner)
        so2 = self._create_sale_order(self.partner2, self.invoice_partner)
        (so1 + so2)._create_invoices()
        self.assertEqual(so1.invoice_ids, so2.invoice_ids)
        self.assertEqual(so1.invoice_ids.partner_sale_id, self.invoice_partner)
