...
class AccountInvoiceAdvanceCompensationWizard(models.TransientModel):
    ...
    invoice_line_id = fields.Many2one(
        comodel_name="account.move.line",
        string="Invoice Line",
        domain="[('move_id', '=', active_id), ('amount_residual', '!=', 0)]",
        required=True,
    ...
    def _prepare_move_vals(self):
        ...
        return {
            ...
            "ref": _("Advance compensation for %s") % self.invoice_line_id.move_id.name,
            "line_ids": [
    ...
    def _process_reconciliation(self, move):
        ...
        compensation_line, advance_comp_line = move.line_ids[0], move.line_ids[1]

        try:
            # Reconcile invoice line
            (self.invoice_line_id + compensation_line).reconcile()

            # Reconcile advance line
            (self.advance_line_id + advance_comp_line).reconcile()
        except Exception as e:
            # Undo any partial reconciliation
            self.invoice_line_id.remove_move_reconcile()
            raise ValidationError(
                _("Reconciliation failed: %s\nPlease verify the accounts and amounts.")
                % str(e)
            ) from e
