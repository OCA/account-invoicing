...
class TestAccountInvoiceMergePurchase(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.invoice_model = cls.env["account.invoice"].with_context(type="in_invoice")
    ...
    def test_picking_multi_purchase_order(self):
        ...
        self.assertEqual(
            self.purchase_order.order_line.mapped("qty_received"),
            [7.0],
        ...
        self.assertEqual(
            self.purchase_order02.state,
            "purchase",
            'Purchase: PO state should be "Purchase"',
        )
        self.assertEqual(
            self.purchase_order02.invoice_status,
            "no",
            'Purchase: PO invoice_status should be "Nothing to Invoice"',
        )
        # I check if there is a picking
        self.assertEqual(
            self.purchase_order02.picking_count,
            1,
            'Purchase: one picking should be created"',
        )
        # I receive picking order
