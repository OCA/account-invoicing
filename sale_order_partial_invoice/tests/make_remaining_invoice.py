...
class TestCrmClaimOperatingUnit(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.adv_pay_inv = self.registry('sale.advance.payment.inv')
    ...
    def _create_product(self, cr, uid, name, list_price, product_type,
        ...
        """Create a Product."""
        product_id = self.product_model.\
            create(cr, uid, {'name': name,
                             'type': product_type,
                             'standard_price': 1.0,
    ...
    def _create_sale_order(self, cr, uid, context):
        ...
        """Create a sale order."""
        stockable_lines = {'product_id': self.stockable_product_id,
                           'product_uom_qty': 3,
                           'name': 'Stockable Product'
                           }
        service_lines = {'product_id': self.service_product_id,
                         'product_uom_qty': 3,
                         'name': 'Service Product'
                         }
        consu_lines = {'product_id': self.consumable_product_id,
                       'product_uom_qty': 12,
        ...
        lines = [(0, 0, stockable_lines),
                 (0, 0, service_lines),
                 (0, 0, consu_lines)]
        sale_id = self.sale_order_model.\
            create(cr, uid, {'partner_id': self.partner_id.id,
        ...
        self.sale_order_model.browse(cr, uid, sale_id, context=None)
        return sale_id
    ...
    def test_security(self):
        ...
        self.assertEqual(len(order.invoice_ids), 1, "It should have 1 invoice")

        """ Check qty_invoiced """
        self.assertEqual(order.order_line[0].qty_invoiced, 1,
                         "qty_invoiced not correctly implemented")
        self.assertEqual(order.order_line[1].qty_invoiced, 1,
                         "qty_invoiced not correctly implemented")
        self.assertEqual(order.order_line[2].qty_invoiced, 4,
                         "qty_invoiced not correctly implemented")

        """ Check qty_delivered """
        self.assertEqual(order.order_line[0].qty_delivered, 1,
                         "qty_delivered not correctly implemented")
        self.assertEqual(order.order_line[1].qty_delivered, 1,
                         "qty_delivered not correctly implemented")
        self.assertEqual(order.order_line[2].qty_delivered, 4,
                         "qty_delivered not correctly implemented")
        ...
        pay_id = self.adv_pay_inv.create(cr, uid,
                                         {'advance_payment_method': 'all'})
        self.adv_pay_inv.create_invoices(cr, uid, [pay_id], context=ctx)

        """Check the invoiced qtys in the invoice """
        order = self.sale_order_model.browse(cr, uid, sale.id)
        self.assertEqual(len(order.invoice_ids), 2, "I should have 2 invoices")
