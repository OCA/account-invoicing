...
class TestAccountMultiBranchCompany(TransactionCase):
    ...
    def test_02_default_branch_journal(self):
        self.journal.write({"branch_id": self.branch2.id})
        move = self._create_move("in_invoice")
        self.assertFalse(move.branch_id)
        move.branch_id = self.branch1.id
        move.journal_id = self.journal.id
        move.action_post()
        # Register with journal default branch
        register_payments = self.register_payment_model.with_context(
            active_model="account.move",
            active_ids=[move.id],
        ...
        payment = register_payments._create_payments()
        self.assertEqual(payment.branch_id, self.branch2)
        self.assertEqual(payment.move_id.branch_id, self.branch2)
