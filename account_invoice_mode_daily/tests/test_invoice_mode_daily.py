...
class TestInvoiceModeDaily(TestInvoiceModeCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.partner.invoicing_mode = "daily"
        cls.partner2.invoicing_mode = "daily"
    ...
    def test_saleorder_with_different_mode_term(self):
        ...
        with tools.mute_logger("odoo.addons.queue_job.models.base"):
            ...
            ).generate_daily_invoices(self.company)
    ...
    def test_split_invoice_by_sale_order(self):
        ...
        self.partner.invoicing_mode = "daily"
    ...
    def test_invoice_daily_cron(self):
        """Check multiple sale order grouped in one invoice"""
        self.deliver_invoice(self.so1)
        self.deliver_invoice(self.so2)
        with tools.mute_logger("odoo.addons.queue_job.models.base"):
            self.SaleOrder.with_context(
                test_queue_job_no_delay=True
            ).cron_generate_daily_invoices()
        self.assertEqual(len(self.so1.invoice_ids), 1)
        self.assertEqual(len(self.so2.invoice_ids), 1)
        # After the cron run the field invoicing_mode_daily_last_execution
        # should be set and therefor _company_daily_invoicing
        # shouldn't return any company
        companies = self.SaleOrder._company_daily_invoicing()
