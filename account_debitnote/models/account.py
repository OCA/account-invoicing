...
class AccountJournal(models.Model):
    ...
    @api.depends(
        "debitnote_sequence_id.use_date_range",
    ...
    def _compute_debitnote_seq_number_next(self):
        ...
        for journal in self:
            if journal.debitnote_sequence_id and journal.debitnote_sequence:
                ...
                journal.debitnote_sequence_number_next = sequence.number_next_actual
            else:
    ...
    def _inverse_debitnote_seq_number_next(self):
        ...
        for journal in self:
            if (
                journal.debitnote_sequence_id
                and journal.debitnote_sequence
        ...
        if vals.get("debitnote_sequence"):
            for journal in self.filtered(
                ...
                journal_vals = {
                    ...
                    "code": journal.code,
                    "debitnote_sequence_number_next": vals.get(
                ...
                journal.debitnote_sequence_id = (
                    self.sudo()
                    ._create_debitnote_sequence(journal_vals, debitnote=True)
                    .id
                )
        return res
    ...
    def _create_debitnote_sequence(self, vals, debitnote=False):
        ...
        seq_name = debitnote and vals["code"] + _(": Debitnote") or vals["code"]
        seq = {
        ...
        seq_date_range.number_next = (
            debitnote
            and vals.get("debitnote_sequence_number_next", 1)
    ...
    def create(self, vals):
        if (
            ...
            and vals.get("debitnote_sequence")
            and not vals.get("debitnote_sequence_id")
        ):
            vals.update(
                {
                    "debitnote_sequence_id": self.sudo()
                    ._create_debitnote_sequence(vals, debitnote=True)
                    .id
                }
            )
