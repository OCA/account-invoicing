...
class AccountInvoice(models.Model):
    ...
    def action_account_change_currency(self):
        ...
        for invoice in self.filtered(lambda x: x.state == "draft"):
            ...
            if not invoice.custom_rate:
                new_rate = rate_skip
            if (
                old_rate_currency == currency
                and new_rate
                and old_rate
                and float_compare(new_rate, old_rate, precision_digits=precision) == 0
            ):
                if not invoice.custom_rate or old_rate == new_rate:
                    continue
                new_rate = rate_skip
                if float_compare(new_rate, old_rate, precision_digits=precision) == 0:
                    continue
    ...
    def _onchange_currency_change_rate(self):
        ...
        if last_currency == self.currency_id and self.get_last_currency_id(True):
            last_currency = self.get_last_currency_id(True)
            if last_currency == self.currency_id:
                last_currency = self.get_last_rate()[0]
        if (
            not (self.currency_id and last_currency)
            or self.currency_id == last_currency
        ):
    ...
    def get_last_currency_id(self, skip_update_currency=False):
        ...
        subtype_id = self.env.ref("account_invoice_change_currency.mt_currency_update")
...
ORDER BY mtv.write_date DESC, mtv.id DESC LIMIT 1"""
    ...
    def get_last_rate(self):
        ...
        if (
            len(last_values) == 1
            and last_values.mail_message_id.subtype_id == subtype_create_id
        ):
    ...
    def get_force_rate_state(self):
        ...
        last_value = (
            self.env["mail.tracking.value"]
            .sudo()
            .search(domain, limit=1, order="write_date desc, id desc")
        )
        return last_value
