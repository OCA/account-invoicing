        ...
        for inv_line in invoice.invoice_line_ids:
            for po_line in po.order_line:
                if inv_line.purchase_line_id == po_line:
                    self.assertEquals(inv_line.price_unit, po_line.price_unit)
                    self.assertEquals(inv_line.discount, po_line.discount)
                    self.assertEquals(inv_line.discount2, po_line.discount2)
                    self.assertEquals(inv_line.discount3, po_line.discount3)
                    self.assertEquals(inv_line.uom_id, po_line.product_uom)
