...
class WizardUpdateInvoiceSupplierinfoLine(models.TransientModel):
    ...
    current_discount2 = fields.Float(
        string="Current Disc. 2 (%)", readonly=True, digits=dp.get_precision("Discount")
    )

    new_discount2 = fields.Float(
        string="New Disc. 2 (%)", required=True, digits=dp.get_precision("Discount")
    )

    current_discount3 = fields.Float(
        string="Current Disc. 3 (%)", readonly=True, digits=dp.get_precision("Discount")
    )

    new_discount3 = fields.Float(
        string="New Disc. 3 (%)", required=True, digits=dp.get_precision("Discount")
    )
