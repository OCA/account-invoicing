...
class InvoiceExcelExport(models.TransientModel):
    ...
    filename = fields.Char(default="Invoice Excel.xls")
    ...
    def export_report_xlsx(self):
        ...
        header_style = easyxf(
            "font:height 200;pattern: pattern solid, "
            "fore_color blue; align: horiz center;font: "
            "color white; font:bold True;"
            "borders: "
            "top thin,left thin,right thin,bottom thin"
        )
        text_left = easyxf(
            "font:height 200; align: horiz left;" "borders: top thin,bottom thin"
        )
        ...
        for inv in self.invoice_ids:
            worksheet.write(i, 0, inv.name or "", text_left)
            if not inv.sale_order_ids.ids:
                i += 1
                ...
                if order.id:
                    ...
                    if order.picking_ids:
                        for pick in order.picking_ids:
                            ...
                            date_pick = pick.date.strftime(
                                DEFAULT_SERVER_DATETIME_FORMAT
                            )
                            pick_date = datetime.strptime(
                                date_pick, DEFAULT_SERVER_DATETIME_FORMAT
                            ).strftime("%d.%m.%Y")
                            ...
                            if pick.partner_id.email:
                                worksheet.write(
                                    i, 7, pick.partner_id.email or "", text_left
                                )
                            worksheet.write(
                                i, 8, pick.carrier_tracking_ref or "", text_left
                            )
                            i += 1
        f_p = io.BytesIO()
        workbook.save(f_p)
        ...
        action = {
            ...
            "url": "/web/content/?model=invoice.excel.export&id="
            + str(self.id)
            + "&field=file_data&download=true&filename=invoice_excel.xls",
            "target": "self",
        }
        return action
