...
class AccountInvoiceLine(models.Model):
    ...
    def _onchange_default_partner_account(self):
        ...
        if (
            ...
            and self.partner_id.auto_update_account_expense
        ):
            if self.account_id != self.partner_id.property_account_expense:
                self.partner_id.write({"property_account_expense": self.account_id.id})
        elif (
            inv_type in ["out_invoice", "out_refund"]
            and self.partner_id.auto_update_account_income
        ):
            if self.account_id != self.partner_id.property_account_income:
                self.partner_id.write({"property_account_income": self.account_id.id})
