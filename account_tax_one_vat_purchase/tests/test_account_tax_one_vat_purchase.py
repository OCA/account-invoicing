...
class TestAccountTaxOneVatPurchase(TestAccountTaxOneVatCommon):
    def test_po_line_without_limitation(self):
        ...
        po_line.tax_ids = [Command.set(self.vat_taxes.ids)]
        self.assertEqual(po_line.tax_ids, self.vat_taxes)
    ...
    def test_po_line_with_limitation_constraint(self):
        ...
        with self.assertRaises(ValidationError, msg=msg):
            po_line.tax_ids = [Command.set(self.vat_taxes.ids)]
        nb_vat_taxes = len(po_line.tax_ids.filtered("is_vat"))
    ...
    def test_po_lines_with_limitation_no_constraint(self):
        ...
        po_line = po.order_line[0]
        po_line.tax_ids = [Command.set(self.mixed_taxes.ids)]
        self.assertEqual(len(po_line.tax_ids), 2)
        nb_vat_taxes = len(po_line.tax_ids.filtered("is_vat"))
