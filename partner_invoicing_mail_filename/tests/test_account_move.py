...
class TestAccountMove(BaseCommon):
    ...
    - Creates invoices assigned to these partners to test filename generation
    """
    ...
    def test_custom_filename_single(self):
        ...
        self.assertIn(self.partner_custom.name.replace("/", "_"), filename)
    ...
    def test_batch_filenames(self):
        ...
        self.assertIn(
            self.partner_custom.name.replace("/", "_"),
            filenames[self.invoice_custom.id],
        )
        self.assertIsNotNone(filenames[self.invoice_default.id])
