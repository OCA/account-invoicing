...
class AccountMove(models.Model):
    ...
    def _onchange_retained_move_ids(self):
        ...
        if self.retained_move_ids:
            # Find the retained move_lines
            retention_account = self.env.company.retention_account_id
            move_lines = self.retained_move_ids.mapped("line_ids").filtered(
    ...
    def _compute_retention_residual_currency(self):
        ...
        for rec in self:
            ...
            rec.retention_residual_currency = rec.retention_amount_currency - abs(
                retained
            )

    @api.constrains("retention_amount_currency")
