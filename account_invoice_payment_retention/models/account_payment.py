...
class AccountPayment(models.Model):
    ...
    @api.depends("journal_id", "invoice_ids", "reconciled_invoice_ids")
        ...
        for rec in self.filtered("enforce_payment_retention"):
            ...
            if not float_is_zero(residual, precision_digits=2):
                raise ValidationError(
                    _(
                        "This payment has retention, please make sure you retain "
                        "amount equal to amount specified in the paying invoices"
                    )
                )
