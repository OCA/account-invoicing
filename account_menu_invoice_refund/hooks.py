...
def post_load_hook():
    ...
    def new_default_get(self, fields):
        ...
        if any(inv.currency_id != invoices[0].currency_id for inv in invoices):
            raise UserError(
                _(
                    "In order to pay multiple invoices "
        ...
        rec.update(
            {
                ...
                "partner_type": (
                    False if multi else MAP_INVOICE_TYPE_PARTNER_TYPE[invoices[0].type]
                ),
                "communication": " ".join(
                    [ref for ref in invoices.mapped("reference") if ref]
                )[:2000],
    ...
    if not hasattr(account_abstract_payment, "default_get_original"):
