            ...
            for key in taxes_grouped:
                for tax_group in tax_groups:
                    if taxes_grouped[key]["tax_id"] == tax_group.id:
                        ...
                        if any(
                            t.get("price_include", False)
                            for t in taxes_recomputed["taxes"]
                        ):
                        ...
                        if float_compare(
                            ...
                            tax_max_diff = (
                                self.env.user.company_id.tax_max_diff_global_rounding_method
                            )
                            if amount_diff > tax_max_diff:
                                raise exceptions.ValidationError(
                                    _(
                                        "Global recompute of tax %s exceeds max "
                                        "tax diff allowed %s"
