...
class TestAccountInvoiceBlockingPayment(SavepointCase):
    ...
    def test_draft2open_blocked(self):
        """
        Test: payment order with blocked invoice
        """
        with self.assertRaises(UserError):
            self.invoice.draft_blocked = True
            self.invoice.blocked = True
            ...
            move_lines = self.account_move_line.search([
                ('blocked', '=', True),
                ('move_id.id', '=', self.invoice.move_id.id)])
            self.assertGreater(len(move_lines), 0)
            pay_line = self.account_payment_line.create({
                'move_line_id': move_lines[0].id,
                'currency_id': self.env.ref('base.EUR').id,
                'partner_id': self.invoice.partner_id.id,
                'communication': 'Test communication'
            })
            pay_order = self.account_payment_order.create({
                'payment_mode_id': self.payment_mode.id,
                'payment_type': 'outbound',
                'payment_line_ids': [(6, 0, [pay_line.id])]
            })
            pay_order.draft2open()

    def test_draft2open_not_blocked(self):
        """
        Test: payment order with not blocked invoice
        """
        self.invoice.action_move_create()
        self.invoice.state = 'open'
        move_lines = self.account_move_line.search([
            ('move_id.id', '=', self.invoice.move_id.id)])
        move_lines.filtered(lambda ml: ml.credit > 0)
        self.assertGreater(len(move_lines), 0)
        pay_line = self.account_payment_line.create({
            'move_line_id': move_lines[0].id,
            'amount_currency': self.invoice.residual,
            'currency_id': self.env.ref('base.EUR').id,
            'partner_id': self.invoice.partner_id.id,
            'communication': 'Test communication'
        })
